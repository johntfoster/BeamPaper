...
Using a decaying exponential nonlocal modulus results in a relationship between \(\mathbf{t}\) and \(\boldsymbol{\sigma}\) 
\begin{align}
\left(1-\tau^2l^2\frac{\partial^2}{\partial x^2}\right)\mathbf{t}&=\boldsymbol{\sigma},\notag
\end{align}
in which \(\tau^2l^2\) is a scale-based material parameter.
For well-behaved \(\mathbf{t}\) and \(\boldsymbol{\sigma}\) and small values of \(\sigma''''\) and \(\tau^2l^2\), we can see that this relationship could be reformulated as
\begin{equation}
\mathbf{t}=\left(1+\tau^2l^2\frac{\partial^2}{\partial x^2}\right)\boldsymbol{\sigma}.\notag
\end{equation}
