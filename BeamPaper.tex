...
The remainder of this introduction reviews other nonlocal models and provides a brief introduction to peridynamics, including state-based models.
...
\Cref{sec:EringenCompare} demonstrates the model's relationship to Eringen's nonlocal elasticity for small peridynamic horizons, and compares other gradient models.
\Cref{sec:Numerical} numerically demonstrates the beam model with simple test cases.
%
...
We can picture a bending resistance based on the nonlocal curvature  \(\bar{\kappa}\), so that 
%
\begin{align}
...
Eringen's model for nonlocal elasticity in \cite{eringen1983differential} begins with a nonlocal modulus that relates the nonlocal stress \(\mathbf{t}\) at a point to the classical (local) stress \(\boldsymbol{\sigma}\) in the nearby material.
\begin{equation}
...
Using a modified bessel function (second kind) nonlocal modulus results in a relationship between \(\mathbf{t}\) and \(\boldsymbol{\sigma}\) 
\begin{align}
\left(1-\tau^2l^2\nabla^2\right)\mathbf{t}&=\boldsymbol{\sigma},\notag
\end{align}
in which \(\tau^2l^2\) is a scale-based material parameter.
For well-behaved \(\mathbf{t}\) and \(\boldsymbol{\sigma}\) and small values of \(\tau^2l^2\) we can see that this relationship could be reformulated as
\begin{equation}
\mathbf{t}=\left(1+\tau^2l^2\nabla^2\right)\boldsymbol{\sigma}.\notag
\end{equation}
...
The similarity is not accidental; Eringen's gradient elasticity is the solution to the integral formulation of the nonlocal stress integral equation just as the peridynamic energy is an integral function of nonlocal displacements.
...
\begin{figure}[h]
  \centering
  \scalebox{.55}{\input{\plotpath/elastic_h20_g2000.pgf}}
...
\begin{figure}[h]
  \centering
  \scalebox{.55}{\input{\plotpath/eppu_h10_g2000.pgf}}
...
\minipage{0.45\textwidth}
  \scalebox{.45}{\input{\plotpath/unloaded2_h10_g2000.pgf}}
\endminipage\hfill
\minipage{0.45\textwidth}%
  \scalebox{.45}{\input{\plotpath/unloaded_n1000_g2000.pgf}}
\endminipage
